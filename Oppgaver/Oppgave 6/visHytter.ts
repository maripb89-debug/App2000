...
type Romtype = "Enkeltrom" | "Dobbeltrom" | "Sovesal";
...
function totalSengerForHytte(hytte: Hytte): number {
...
function laveastePris(hytte: Hytte): number | null {
  if (hytte.rom.length === 0) return null;
...
function hentHytteHtml(hytte: Hytte): string {
...
function visHytter(data: HytteData, containerId: string): void {
